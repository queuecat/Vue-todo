...
todoRouter.get('/task', async (req, res) => {
  try {
    ...
    res.status(400).send({ message: `查询失败：${error.message}` })
  }
})
// 获取已完成任务列表
todoRouter.get('/taskDone', async (req, res) => {
  try {
...
// 获取未完成任务列表
todoRouter.get('/taskUndone', async (req, res) => {
  try {
...
todoRouter.post('/addTask', async (req, res) => {
  ...
  try {
    ...
    res.status(400).send({ message: `添加失败：${err.message}` })
  }
})
...
todoRouter.get('/deleteTask', async (req, res) => {
  ...
  try {
    ...
    res.status(400).send({ message: `删除失败：${err.message}` })
  }
})
...
todoRouter.get('/clearTask', async (req, res) => {
  ...
  try {
    ...
    res.status(400).send({ message: `清空失败：${err.message}` })
  }
})
